...
import { 
  Terminal, Globe, ShieldAlert, BrainCircuit, Cpu, Server, 
  Play, ArrowLeft, ChevronRight, Clock, HelpCircle, CheckCircle, XCircle, RotateCcw
} from "lucide-react";
    ...
    topics: [
      {
        ...
        time: "15 Mins",
        questions: [
          { id: 1, text: "What is the time complexity to access an element in an array?", options: ["O(1)", "O(n)", "O(log n)", "O(n^2)"], correctAnswer: 0 },
          { id: 2, text: "Which string operation is immutable in Java/Python?", options: ["Append", "Modification", "Concat", "Slice"], correctAnswer: 1 },
        ]
      },
      {
        id: "dsa-trees",
        ...
        time: "25 Mins",
        questions: [
          { id: 1, text: "In a BST, the left child is always...", options: ["Larger than root", "Smaller than root", "Equal to root", "Random"], correctAnswer: 1 },
          { id: 2, text: "Which traversal visits the root node last?", options: ["Pre-order", "In-order", "Post-order", "Level-order"], correctAnswer: 2 },
        ]
      }
    ]
  },
    ...
    topics: [
      {
        ...
        time: "20 Mins",
        questions: [
          { id: 1, text: "Which hook is used for side effects?", options: ["useState", "useEffect", "useContext", "useReducer"], correctAnswer: 1 },
        ]
      },
      {
        id: "web-css",
        ...
        time: "15 Mins",
        questions: [
          { id: 1, text: "Which property changes the axis in Flexbox?", options: ["justify-content", "align-items", "flex-direction", "flex-wrap"], correctAnswer: 2 },
        ]
      }
    ]
  },
    ...
    topics: [
      {
        ...
        time: "30 Mins",
        questions: [
          { id: 1, text: "What protects against SQL Injection?", options: ["Prepared Statements", "HTTPS", "Firewalls", "Hashing"], correctAnswer: 0 },
        ]
      }
    ]
  },
    ...
    topics: [
        {
            ...
            difficulty: "Medium",
            time: "20 Mins",
            questions: [{ id: 1, text: "Which is a supervised learning algorithm?", options: ["K-Means", "Linear Regression", "Apriori", "PCA"], correctAnswer: 1 }]
        }
    ]
  },
  Aptitude: {
    ...
    topics: [
        {
            ...
            difficulty: "Medium",
            time: "20 Mins",
            questions: [{ id: 1, text: "Find the odd one out: 3, 5, 7, 9", options: ["3", "5", "7", "9"], correctAnswer: 3 }]
        }
    ]
  },
  System: {
  ...
  if (showResult) {
    ...
    return (
      <div className="max-w-2xl mx-auto pt-10">
        <Button variant="ghost" onClick={onBack} className="mb-6 text-slate-400 hover:text-white">
        ...
        <Card className="bg-[#0b1021] border-slate-800 text-white">
          <CardContent className="p-8 text-center space-y-6">
            <div className="w-24 h-24 mx-auto rounded-full bg-slate-800/50 flex items-center justify-center border border-slate-700">
              {percentage >= 70 ? <CheckCircle className="w-12 h-12 text-green-500" /> : <XCircle className="w-12 h-12 text-red-500" />}
            </div>
            ...
            <div className="py-6 bg-slate-900/50 rounded-xl border border-slate-800">
              ...
              <span className="text-2xl text-slate-500"> / {topic.questions.length}</span>
              <p className="mt-2 text-sm text-slate-400 uppercase tracking-widest">Your Score</p>
            </div>
            <Button onClick={onBack} className="w-full bg-slate-800 hover:bg-slate-700 text-white">
      ...
      <Card className="bg-[#0b1021] border-slate-800 text-white">
        <CardContent className="p-8">
            ...
            <div className="flex justify-end">
                <Button onClick={handleNext} disabled={selectedOption === null} className="bg-blue-600 hover:bg-blue-700 text-white px-8">
                    {currentQuestion === topic.questions.length - 1 ? "Finish Test" : "Next Question"}
                </Button>
            </div>
        </CardContent>
      </Card>
      ...
      <div className="min-h-screen bg-[#02040a] p-6 text-slate-200">
        <div className="max-w-5xl mx-auto space-y-8">
            ...
            <div className="flex items-center gap-4 mb-8">
                <Button variant="ghost" size="icon" onClick={() => setSelectedSubject(null)} className="text-slate-400 hover:bg-slate-800 hover:text-white">
                    <ArrowLeft className="w-5 h-5" />
                </Button>
                <div>
                    <h1 className="text-2xl font-bold text-white flex items-center gap-2">
                        <selectedSubject.icon className={`w-6 h-6 ${selectedSubject.color}`} />
                        {selectedSubject.title}
                    </h1>
                    <p className="text-slate-500">Select a module to begin your assessment</p>
                </div>
            </div>
            ...
            {selectedSubject.topics.length === 0 && (
                <div className="text-center py-20 bg-[#0b1021] border border-slate-800 rounded-2xl border-dashed">
                    <p className="text-slate-500">No modules available for this subject yet.</p>
                    <Button variant="link" onClick={() => setSelectedSubject(null)}>Go Back</Button>
                </div>
            )}
                    ...
                    >
                        <div className="space-y-2 flex-1">
                            ...
                            <div className="flex items-center gap-4 text-xs text-slate-500 font-medium pt-1">
                                <span className="flex items-center gap-1"><HelpCircle className="w-3 h-3" /> {topic.questions.length} Questions</span>
      ...
      <div className="max-w-6xl mx-auto space-y-12 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {Object.values(SUBJECT_DATA).map((subject) => (
            ...
            >
               {/* Icon */}
              <div className={`w-12 h-12 rounded-lg flex items-center justify-center mb-4 ${subject.bg} border ${subject.border}`}>
              ...
              {/* Action Text */}
              <div className={`flex items-center text-sm font-medium ${subject.topics.length > 0 ? "text-blue-500 group-hover:translate-x-1 transition-transform" : "text-slate-500"}`}>
                 {subject.topics.length > 0 ? "View Topics" : "Locked"} 
                 {subject.topics.length > 0 && <ChevronRight className="w-4 h-4 ml-1" />}
              </div>
            </div>
          ))}
        </div>
