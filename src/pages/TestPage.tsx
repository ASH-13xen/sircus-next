      ...
      <div className="container mx-auto px-4 py-12 max-w-6xl space-y-16">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end gap-6 border-b border-slate-800 pb-8">
          <div>
            ...
            <p className="text-slate-400">Manage your technical assessments and live coding sessions.</p>
          </div>

          {isProctor && (
            <Link href="/test/proctor">
              <Button variant="destructive" className="bg-red-900/20 hover:bg-red-900/40 text-red-400 border border-red-900/50 shadow-[0_0_15px_rgba(220,38,38,0.2)] gap-2">
        ...
        <section>
          <div className="flex items-center gap-2 mb-6">
            ...
            <h2 className="text-2xl font-bold text-white">My Scheduled Sessions</h2>
          </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {upcomingTests.length === 0 && (
              <div className="col-span-full py-16 bg-[#0b1021] border border-dashed border-slate-800 rounded-2xl flex flex-col items-center justify-center text-center">
                ...
                <p className="text-slate-600 text-sm mt-1">Register for an open slot below.</p>
              </div>
            )}
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {availableTests.length === 0 && (
              <div className="col-span-full text-center py-12">
                <p className="text-slate-500">No open slots available right now.</p>
              </div>
            )}
...
function TestCard({ test, type, onRegister }: TestCardProps) {
  ...
  const borderColor = isLive ? "border-red-500/50" : isPast ? "border-slate-800" : "border-slate-700";
  const glowClass = isLive ? "shadow-[0_0_20px_rgba(239,68,68,0.15)]" : "hover:shadow-lg hover:shadow-blue-900/10";
  ...
  return (
    ...
    >
      ...
      <CardHeader className="pb-3">
        ...
        <CardTitle className={`text-xl font-bold tracking-tight ${isPast ? "text-slate-400" : "text-white"}`}>
            {test.title}
        </CardTitle>
      ...
      <CardContent className="flex-1 space-y-4">
        ...
        {type === "past" && test.myScore !== undefined ? (
          <div className="mt-4 p-3 bg-slate-900 border border-slate-800 rounded-lg flex justify-between items-center">
             <span className="text-xs text-slate-500 uppercase font-bold">Result</span>
             <div className="flex items-baseline gap-1">
                <span className={`text-xl font-bold font-mono ${test.myScore > (test.maxPoints * 0.7) ? 'text-emerald-400' : 'text-yellow-400'}`}>
                    {test.myScore}
                </span>
                <span className="text-xs text-slate-600 font-mono">/ {test.maxPoints}</span>
             </div>
          </div>
        ) : (
             <div className="flex items-center gap-2 text-xs font-medium text-slate-500 pt-1">
                ...
                <span>Max Score: <span className="text-slate-300">{test.maxPoints}</span></span>
            </div>
        )}
      ...
      <CardFooter className="pt-4 border-t border-slate-800/50">
          ...
          <div className="w-full">
            {isTimeReached || isLive ? (
              ...
              <Button className="w-full bg-slate-800 text-slate-400 border border-slate-700 cursor-not-allowed hover:bg-slate-800">
                <span className="mr-2 opacity-50"><Play className="w-3 h-3" /></span>
        ...
        {type === "past" && (
             <Button variant="outline" disabled className="w-full border-slate-800 text-slate-600 bg-transparent">
                Completed
             </Button>
        )}
      </CardFooter>
