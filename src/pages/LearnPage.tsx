...
import { 
  ArrowRight, Shield, Code, Database, Brain, BookOpen, Layers, Lock,
  X, Send, Tent, MessageSquare, Bot
} from "lucide-react";
...
const LearnPage = () => {
  ...
  const [messages, setMessages] = useState([
    { id: 1, text: "Welcome to the arena! I am RingMaster. Select a path above or ask me for guidance.", sender: "bot" }
  ]);
  ...
  const handleSend = (e: React.FormEvent) => {
    ...
    setMessages((prev) => [...prev, { id: Date.now(), text: inputMsg, sender: "user" }]);
    setInputMsg("");
    
    setTimeout(() => {
        setMessages((prev) => [...prev, { id: Date.now() + 1, text: "I'm analyzing your request... (Demo Mode)", sender: "bot" }]);
    }, 1000);
  };
    ...
    <div className="min-h-screen bg-[#02040a] relative isolate font-sans selection:bg-blue-500/30 selection:text-blue-200">
      ...
      <div className="max-w-6xl mx-auto pt-16 pb-12 px-6">
        <h1 className="text-4xl md:text-5xl font-bold tracking-tight mb-4 text-white">
          Learning <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-emerald-400">Paths</span>
        </h1>
        <p className="text-slate-400 text-lg max-w-2xl">
          Select a domain to start your journey. Each path is curated to take you from beginner to advanced mastery.
        </p>
      </div>
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 px-6 pb-24">
        {subjects.map((subject, idx) => {
          ...
          if (isLocked) {
            return (
              <div key={idx} className="group relative h-full p-1 rounded-2xl bg-gradient-to-b from-yellow-500/20 to-transparent shadow-lg shadow-yellow-900/10">
                <div className="h-full bg-[#0b1021] rounded-xl p-6 relative overflow-hidden flex flex-col border border-yellow-500/30">
                    ...
                    <div className="relative z-10 flex flex-col h-full">
                        ...
                        <p className="text-slate-400 text-sm mb-6 flex-grow">
                            Unlock exclusive access to advanced Aptitude & Reasoning modules.
                        </p>
          ...
          const CardContent = (
            ...
            `}>
              ...
              <div className="relative z-10 flex flex-col h-full">
                {/* Icon Box */}
                <div className={`mb-5 p-3 w-fit rounded-lg border ${subject.bg} ${subject.border} ${subject.disabled ? 'grayscale opacity-50' : ''}`}>
                ...
                <div className="mt-auto pt-4 border-t border-slate-800/50 flex items-center justify-between">
                  {subject.disabled ? (
                    <span className="text-slate-600 uppercase tracking-widest text-[10px] font-bold border border-slate-800 px-2 py-1 rounded">In Development</span>
                  ) : (
          ...
          <div className="w-[320px] sm:w-[380px] h-[500px] bg-[#0b1021] border border-slate-700 rounded-2xl shadow-2xl overflow-hidden flex flex-col animate-in slide-in-from-bottom-5 fade-in duration-300 backdrop-blur-md">
            ...
            <div className="p-4 bg-slate-900/80 border-b border-slate-700 flex justify-between items-center backdrop-blur-md">
              <div className="flex items-center gap-3">
                ...
                <div>
                    <h3 className="font-bold text-white text-sm tracking-wide">RingMaster <span className="text-blue-500">AI</span></h3>
